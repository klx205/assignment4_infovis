...
const Charts = () => {
    ...
    const [tooltipX, setTooltipX] = useState(0);  // Hook for X coordinate
    const [tooltipY, setTooltipY] = useState(0);  // Hook for Y coordinate
...
//Hint: use d3.scaleBand for xScaleBar
    ...
    .domain(data.map(d => d.station)) // Assuming 'start' is the category (like months)
    .range([0, innerWidth])
    .padding(0.1); // Space between the bars
    const yScaleBar = d3.scaleLinear()
    .domain([0, 4000]) // Scale based on the trip duration
    .range([innerHeightBar, 0]) // Map values to height
    .nice();
    ...
    return (
        <Container >
            ...
            <Row className='justify-content-md-center'>
                <Col>
                    <svg width={WIDTH} height={HEIGHT}>
                        ...
                        height={innerHeightScatter} width={innerWidth} hoveredStation={hoveredStation} // Pass state
                        setHoveredStation={setHoveredStation} // Pass setter function 
                        setTooltipX={setTooltipX} setTooltipY={setTooltipY}  // Passing tooltip position setters
                        />
                    </svg>
                </Col>
                <Col>
                    <svg width={WIDTH} height={HEIGHT}>
                        ...
                        yScale={yScaleBar} height={innerHeightBar} width={innerWidth} hoveredStation={hoveredStation} // Pass state
                        setHoveredStation={setHoveredStation} // Pass setter function
                        />
                    </svg>
                </Col>
            ...
            */}
            {hoveredStation && (
                <Tooltip 
                d={data.find(d => d.station === hoveredStation)}  // Passing the hovered station's data
                x={tooltipX} 
                y={tooltipY} 
            />
)}
